...
export function CreatedDateFilter({ createdDateFilter, setFilter }: StatusFilterProps) {
  ...
  return (
    <Popover defaultOpen={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="text-xs font-medium">Data de criação: </span>
          {createdDateFilter?.to && createdDateFilter?.from ? 
            <>
                {format(createdDateFilter.from!, "dd/MM/yyyy")} {" "}
                {format(createdDateFilter.to, "dd/MM/yyyy") }
            </>
            :
      ...
      <PopoverContent align="start" className="w-full space-y-6">
        <div>
          <p className="text-foreground text-sm font-bold">Filtrar por data de criação:</p>
        </div>
        <Calendar
            mode="range"
            locale={ptBR}
